...
export const applyPimProductProjectionStep = createStep(
  ...
  async (input: Input, { container }) => {
    ...
    const receipts = await connector.listPimEventReceipts({
      id: input.receipt_id,
      source: input.source,
    });
    ...
    const payload = PimProductRevisionSchema.parse(sync.latest_payload);
    ...
    try {
      ...
      if (createdProductId) {
        await productModuleService.deleteProducts([createdProductId]);
      }
