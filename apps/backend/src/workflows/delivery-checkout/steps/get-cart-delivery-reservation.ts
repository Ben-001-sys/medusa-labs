...
>(
  ...
  async ({ cart_id }: Input, { container }) => {
    ...
    const reservation = reservations.find(
      (item) =>
        item.status === DeliverySlotReservationStatus.ACTIVE ||
        item.status === DeliverySlotReservationStatus.CHECKOUT_PENDING ||
        item.status === DeliverySlotReservationStatus.CONFIRMED,
    );
