...
type ClaimDeliverySlotForCheckoutStepOutput = {
  reservation: Awaited<
    ReturnType<DeliverySlotModuleService["retrieveDeliverySlotReservation"]>
  >;
...
>(
  ...
  async ({ cart_id, reservation_id }: Input, { container }) => {
    ...
    const reservation =
      await deliverySlotService.retrieveDeliverySlotReservation(reservation_id);
    ...
    if (reservation.status === DeliverySlotReservationStatus.CONFIRMED) {
    ...
    const slot = await deliverySlotService.retrieveDeliverySlot(
      reservation.slot_id,
    );
    ...
    const checkoutExpiresAt = new Date(now.getTime() + 5 * 60 * 1000);

    const updated = await deliverySlotService.updateDeliverySlotReservations({
      id: reservation.id,
